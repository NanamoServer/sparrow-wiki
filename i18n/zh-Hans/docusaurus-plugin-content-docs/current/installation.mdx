...
<Admonition type="warning" title="存储服务版本">
...
<PluginFileTree
  title="Sparrow"
  initialTreeData={[
    { id: "libs", name: "libs", hoverText: "自动下载和加载的运行时依赖", children: [] },
    { id: "translations", name: "translations", hoverText: "命令反馈和界面的多语言文本", children: [] },
    { id: "config.yml", name: "config.yml", isLeaf: true, hoverText: "Redis、数据库、语言和文本解析选项" },
    { id: "server.yml", name: "server.yml", isLeaf: true, hoverText: "本服在集群中的唯一 ID" },
    { id: "commands.yml", name: "commands.yml", isLeaf: true, hoverText: "命令开关、入口和权限，修改后重启" },
    { id: "features.yml", name: "features.yml", isLeaf: true, hoverText: "各功能模块的开关和设置" },
  ]}
/>
...
控制台语言可以通过 `config.yml` 的 `forced-locale: zh_CN` 设置。更多命令见[权限命令速查表](./commands.mdx)，每个功能的配置见左侧功能分组。
